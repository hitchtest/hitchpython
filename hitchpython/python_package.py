...
class PythonPackage(HitchPackage):
    ...
    def build(self):
        ...
        if "PIP_REQUIRE_VIRTUALENV" in env_vars:
            env_vars.pop("PIP_REQUIRE_VIRTUALENV")

        try:
            self.base_python_bin_directory = path.join(
                self.base_python_main_directory, "bin"
            )
            if not path.exists(self.base_python_main_directory):
                try:
                    checks.internet_detected_after(5)
                except checks.HitchEnvironmentException:
                    raise HitchPythonBuildError(
                        "Internet access not detected -- cannot install python environment."
                    )
                python_build.build.python_build(
                    self.python_version, self.base_python_main_directory
                )
                easy_install = path.join(
                    self.base_python_bin_directory, "easy_install"
                )
                check_call([easy_install, "--upgrade", "setuptools"])
                check_call([easy_install, "--upgrade", "pip"])
                check_call(
                    [
                        path.join(self.base_python_bin_directory, "pip"),
                        "install", "virtualenv", "-U"
                    ],
                    env=env_vars
                )

                # Blow away virtualenv if it exists
            ...
            if not path.exists(self.directory):
                check_call(
                    [path.join(self.base_python_bin_directory, "virtualenv"), self.directory]
                )
            ...
            raise HitchPythonBuildError((
                ...
                "Your OS, OS version and package manager. "
                "Please also copy and paste everything above this exception."
            ))
